...
- Если агент работает на Windows и вносит большие правки, он не должен копить один огромный patch до конца фазы. Изменения нужно применять поэтапно, маленькими кусками.
...
### 15.10 Большие patch-правки на Windows

- на Windows инструмент применения patch может падать с ошибкой вроде `Io(Os { code: 206, kind: InvalidFilename, message: "The filename or extension is too long." })`
- типичный сценарий: агент долго читает и готовит большой diff, а затем падает в конце, когда пытается применить слишком крупное изменение одним вызовом
- предпочтительный путь:
  - не копить один монолитный patch до конца задачи
  - применять изменения маленькими шагами сразу после локального анализа участка кода
  - не заменять одним patch-вызовом целиком большой HTML/CSS/JS-шаблон, длинный markdown-документ или несколько крупных файлов сразу
  - для больших правок разбивать работу как минимум на базовый каркас, отдельные секции, затем точечные добивки
  - после каждого заметного patch-шага перечитывать изменённый файл и только потом переходить дальше
  - если patch уже упал с `code: 206` или `InvalidFilename`, не повторять тот же большой diff, а сразу дробить его на меньшие части
- это особенно важно для файлов в `templates/` и крупных документов в `docs/`
